    ...
    <article>
      <header className="page-shell pb-14 pt-24 sm:pb-20">
        ...
        <dl className="mt-12 grid gap-7 border-y border-border py-7 sm:grid-cols-3">
          <div>
            ...
            <dd className="mt-2 font-medium">{metadata.role || "Engineer"}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-widest text-muted-foreground">Period</dt>
            <dd className="mt-2 font-medium">{metadata.period || metadata.publishedAt}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-widest text-muted-foreground">Services</dt>
            <dd className="mt-2 font-medium">{metadata.services?.join(", ")}</dd>
          </div>
        </dl>
      </header>
      ...
      {(metadata.coverImage || metadata.image) && (
        <div className="page-shell">
          <div className="relative flex aspect-[16/8] items-center justify-center overflow-hidden rounded-panel bg-secondary">
            <Image
              ...
              className={
                metadata.visualStyle === "brand" ? "object-contain p-20 sm:p-32" : "object-cover"
              }
            />
          </div>
        </div>
      ...
      {metadata.metrics && metadata.metrics.length > 0 && (
        <section aria-label="Project outcomes" className="page-shell py-14">
          <div className="grid grid-cols-1 gap-px overflow-hidden rounded-2xl border border-border bg-border sm:grid-cols-3">
            {metadata.metrics.map((metric) => (
              <div key={metric.label} className="bg-card p-7">
      ...
      <section className="border-t border-border">
        <div className="page-shell py-20 text-center sm:py-28">
          ...
          <h2 className="section-title mx-auto mt-4 max-w-3xl">
            Working through a product idea of your own?
          </h2>

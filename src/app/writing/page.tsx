...
export const metadata: Metadata = {
  title: "Writing",
  description:
    "Practical notes from Richie Budijono about building and shipping software products.",
  ...
  return (
    <section className="page-shell pb-28 pt-24">
      ...
      <h1 className="page-title reveal-header delay-1 mt-5 max-w-4xl">
        Notes from building, learning, and occasionally getting it wrong first.
      </h1>
      <p className="body-large reveal-header delay-2 mt-8 max-w-2xl">
        Technical lessons and product observations from work in mobile, web, cloud, and AI.
      </p>
      <div className="mt-16 border-t border-border">
        {posts.length === 0 && (
          <p className="py-8 text-muted-foreground">No posts at the moment — stay tuned.</p>
        )}
        {posts.map((post) => (
          <Link
            key={post.slug}
            href={`/writing/${post.slug}`}
            className="group grid gap-4 border-b border-border py-8 sm:grid-cols-[1fr_auto] sm:items-center"
          >
            <div>
